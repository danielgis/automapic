...
param = arcpy.GetParameterAsText(2)
...
campo_dominio = [[x[0],x[3]] for x in arcpy.da.SearchCursor(tabla_relacion_campos,["campo_fc", "tipo", "largo", "dominio"])]
...
def crear_tablas(gdb):
	if not arcpy.Exists(nombre_tabla):
		arcpy.CreateTable_management(gdb, _nombre_tabla)

	if not arcpy.Exists(nombre_tabla_corregida):
		arcpy.CreateTable_management(gdb, _nombre_tabla_corregida)

	if not arcpy.Exists(nombre_fc):
		...
		with arcpy.da.SearchCursor(_tabla_relacion_campos,["campo_fc", "tipo", "largo", "dominio"]) as cursor:
			for row in cursor:
				if 'TABLA' in capa:
					...
					arcpy.AddField_management(in_table = capa ,field_name= row[0] ,field_type = row[1], field_length= row[2], field_domain= row[3])
	except:
		print "ya existen los campos para el objeto {0}".format(capa)

def crear_capas_auxiliares(gdb):
    crear_tablas(gdb)
	agregarcampos(nombre_tabla)
	agregarcampos(nombre_fc)
	agregarcampos(nombre_tabla_corregida)
...
def ingresar_datos(excel_ingreso):
	...
	df_a[u'Código']= df_a[u'Código']+"_A"
	df_a["TEMPORADA"]= "Avenida"
	df_e[u'Código']= df_e[u'Código']+"_E"
...
def getvalues_dominio(nom_dominio):
	...
	with arcpy.da.SearchCursor(tabla_dominios,["DOMINIO", "KEY", "VALUE"],where_clause = "DOMINIO ='{}'".format(nom_dominio)) as cursor:
		for row in cursor:
